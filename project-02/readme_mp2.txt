...
setwd("Z:/All Dads Stuff/6. My Stuff/1. Vic School/4. Masters/5. Data Viz/Mini Project 2/Miranda_DataViz_MP2")

# Load and clean internet user data
national_counties <- read_sf("data/2023_TIGER_Line_National_Counties/tl_2023_us_county.shp")
texas_counties <- national_counties %>%
  filter(STATEFP == 48)

# Read in additional files
crosswalk <- read_excel("data/county_zip_tx.xlsx")
agi_2011 <- read_excel("data/iitr_2011_cleaned.xlsx")
agi_2016 <- read_excel("data/iitr_2016_cleaned.xlsx")
agi_2021 <- read_excel("data/iitr_2021_cleaned.xlsx")
...
## I'm trying to combine my interactive plot and the map
```{r}
